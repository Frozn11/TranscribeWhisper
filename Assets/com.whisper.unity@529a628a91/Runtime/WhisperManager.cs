...
namespace Whisper
{
    ...
    public class WhisperManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Clears the loaded model from memory to allow for re-initialization (e.g., switching GPU/CPU).
        /// </summary>
        public void DisposeModel()
        {
            if (_whisper != null)
            {
                // Since WhisperWrapper doesn't have .Dispose(), 
                // we just detach the handlers and null it out.
                ...
                // This is important: clear the dispatcher so old tasks don't linger
                _dispatcher.Update(); 
        
                LogUtils.Log("Whisper model reference cleared. Ready for re-initialization.");
            }
        }
        ...
        public async Task<WhisperResult> GetTextAsync(AudioClip clip, System.Threading.CancellationToken token = default)
        {
            ...
            // create a task for the whisper inference
            var task = _whisper.GetTextAsync(clip, _params);
            ...
            using (token.Register(() => tcs.TrySetResult(true)))
            {
                ...
                {
                    // If the cancellation task finished first, throw exception
        ...
        /// <returns>Full audio transcript. Null if transcription failed.</returns>
        public async Task<WhisperResult> GetTextAsync(float[] samples, int frequency, int channels, System.Threading.CancellationToken token = default)
        {
            var isLoaded = await CheckIfLoaded();
            if (!isLoaded)
                return null;
            ...
            var task = _whisper.GetTextAsync(samples, frequency, channels, _params);
        ...
        public async void SetGpu(bool value) 
        {
            if (useGpu == value && IsLoaded) return;

            useGpu = value;

            if (IsLoaded)
            {
                LogUtils.Log($"Hardware change detected. Reloading Whisper with GPU: {value}");
                DisposeModel(); // Now this won't throw an error!
                await InitModel();
            }
        }
    }
